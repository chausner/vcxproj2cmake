...
public class ConverterTests
{
    ...
    public class DryRunTests
    {
        ...
        public void Given_SolutionWithTwoEmptyProjects_When_ConvertedWithDryRun_Then_LogsExpectedOutputAndWritesNoFiles()
        {
            ...
            Assert.Contains($"Generated output for {Path.Combine(Environment.CurrentDirectory, @"EmptyProject1\CMakeLists.txt")}", logs);
            Assert.Contains("    add_executable(EmptyProject1", logs);
            Assert.Contains($"Generated output for {Path.Combine(Environment.CurrentDirectory, @"EmptyProject2\CMakeLists.txt")}", logs);
            Assert.Contains("    add_executable(EmptyProject2", logs);
            Assert.Contains($"Generated output for {Path.Combine(Environment.CurrentDirectory, "CMakeLists.txt")}", logs);
